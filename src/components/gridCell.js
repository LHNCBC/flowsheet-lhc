      ...
      if (columnIndex === 0) {
        if (dataRow.isTempHeader) {
          ...
          if (dataRow.isEqClassRow) {
            ...
            content = <span className="item-name"><span className="eq-exp-col-button" onClick={() => eqExpColFunc(dataRow.key)}><Icon type="column-height" /></span> {dataRow.displayName} {dataRow.isEqClassRow ? ' [' + Object.keys(dataRow.eqClassItems).length + ']' : ''}</span>
          }
